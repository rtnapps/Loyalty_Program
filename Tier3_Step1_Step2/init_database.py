...
def init_database():
    ...
    try:
        ...
        # Check if required columns exist (for existing databases that need migration)
        try:
            # Only check if customer_profiles table exists
            cursor.execute("""
                SELECT name FROM sqlite_master 
            ...
            if table_exists:
                ...
                if 'format_type' not in columns:
                    missing_columns.append('format_type')
                if 'avt_verified' not in columns:
                    missing_columns.append('avt_verified')
                if 'eaiv_verified' not in columns:
                    missing_columns.append('eaiv_verified')
                if 'cid_customer_id' not in columns:
                    missing_columns.append('cid_customer_id')
                
                if missing_columns:
                    print(f"⚠️  Note: Missing columns: {', '.join(missing_columns)}")
                    print("   Run migration scripts to add them:")
                    if 'format_type' in missing_columns:
                        print("   - python migrate_add_format_type.py")
                    if 'avt_verified' in missing_columns or 'eaiv_verified' in missing_columns:
                        print("   - python migrate_add_age_fields.py")
                    if 'cid_customer_id' in missing_columns:
                        print("   - python migrate_add_cid_customer_id.py")
                else:
                    print("✅ All required columns verified (format_type, avt_verified, eaiv_verified, cid_customer_id)")
